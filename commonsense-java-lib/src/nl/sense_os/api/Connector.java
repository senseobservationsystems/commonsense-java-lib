...
public class Connector {
	...
	/**
	 * Tries to login using the username and password from the private
	 * preferences and updates the {@link #isLoggedIn} status accordingly. Can
	 * also be called from Activities that are bound to the service.
	 * 
	 * @return 0 if login completed successfully, -2 if login was forbidden, and
	...
	public static int login(Credentials c) {
		...
		if ((c.getUsername() != null) && (c.getPassword() != null)) {
			...
			System.out
					.println("Cannot login: username or password unavailable");
		}
	 ...
	 * After login, the cookie containing the session ID is stored in the
	 * preferences.
	 * 
	 * @param context
	...
	public static int login(String username, String password)
		...
		final String url = Params.General.devMode ? SenseUrls.DEV_LOGIN
				: SenseUrls.LOGIN;
		...
		if ("403".equalsIgnoreCase(responseCode)) {
			System.out
					.println("CommonSense login refused! Response: forbidden!");
			result = -2;
		} else if (!"200".equalsIgnoreCase(responseCode)) {
			System.out.println("CommonSense login failed! Response: "
					+ responseCode);
			result = -1;
		} else {
		...
		System.out
				.println("----------------ACHIEVEMENT UNLOCKED, NEW ITEM ACQUIRED---------------------");
		...
		if (result == 0 && response.get("set-cookie") == null) {
			...
			System.out
					.println("CommonSense login failed: no cookie received?!");
			result = -1;
		}
	 ...
	 * @return Map with "content" and "http response code" fields, plus fields
	 *         for all response headers.
	 * @throws IOException
	 */
	public static Map<String, String> request(String urlString,
		...
		while (retry < 3) {
			...
			try {
				...
				if (null != content) {
					...
					if (compress) {
						...
						urlConnection.setRequestProperty("Transfer-Encoding",
								"chunked");
						urlConnection.setRequestProperty("Content-Encoding",
								"gzip");
						...
						// set content size
						urlConnection.setFixedLengthStreamingMode(content
								.toString().length());
				...
				// sleep and retry if 509 (bandwidth limited)
				if (urlConnection.getResponseCode() == 509) {
					Thread.sleep(60000);
					continue;
				}
				...
				List<String> value;
				for (Entry<String, List<String>> entry : headerFields
						.entrySet()) {
					key = entry.getKey();
					...
					if (null != key && null != value) {
						...
						// Log.d(TAG, "Header field '" + key + "': '" +
						// valueString + "'");
						result.put(key, valueString);
					} else {
						// Log.d(TAG, "Skipped header field '" + key + "': '" +
						// value + "'");
					}
				}
	 ...
	 * @throws IOException
	 */
	public static Map<String, String> requestDeletion(String urlString,
